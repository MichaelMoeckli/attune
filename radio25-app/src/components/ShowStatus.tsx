...
export default function ShowStatus({ showResult, isGenerating, error }: ShowStatusProps) {
  if (error) {
    return (
  ...
  if (isGenerating && !showResult) {
    return (
      <div className="w-full max-w-md rounded-lg bg-zinc-800/50 p-4 text-center">
        ...
        <p className="mt-1 text-xs text-zinc-500">Nachrichten, Wetter und Moderation werden generiert</p>
      </div>
    );
  }
    ...
    return (
      <div className="w-full max-w-md rounded-lg bg-zinc-800/50 p-4 text-center">
        ...
        <p className="mt-1 text-xs text-zinc-500">
          {showResult.segments.length} Segmente
        </p>
      </div>
    );
  }
