...
- ✅ **Runtime source check** — `checkSpokenSources()` ([attune-app/src/lib/source-check.ts:14](attune-app/src/lib/source-check.ts#L14)) scans news segments for known outlets (NZZ, SRF, Tagesanzeiger, Blick, Watson, 20 Minuten); wired into orchestrator ([attune-app/src/lib/orchestrator.ts](attune-app/src/lib/orchestrator.ts)) and surfaced via `segment.sourceCheck.unverifiedMention` warning in the player UI

### 2.3 Speech synthesis (TTS)

- ✅ **`tts.ts`** — ElevenLabs + `eleven_multilingual_v2` ([attune-app/src/services/tts.ts:65](attune-app/src/services/tts.ts#L65), `output_format: 'mp3_44100_128'` L. 66)
- ✅ **Voice IDs per moderation style** — `STYLE_ENV_VAR` map + `resolveVoiceId()` ([attune-app/src/services/tts.ts:5-23](attune-app/src/services/tts.ts#L5-L23)) resolve `ELEVENLABS_VOICE_ID_FORMAL` / `_CASUAL` / `_ENERGETIC` per `voiceStyle`, with fallback to `ELEVENLABS_VOICE_ID`. `.env.example` lists all four vars.
- ✅ Audio output as inline data URL (Vercel read-only FS at runtime, [tts.ts:78](attune-app/src/services/tts.ts#L78))
...
- ✅ **Topic diversification** — `capPerTopic(articles, max=2)` in [attune-app/src/services/news.ts](attune-app/src/services/news.ts) caps each topic at 2 articles; topic origin is tracked via the RSS_FEEDS reverse mapping during fetch
- ✅ **Time-of-day context** — two programmatic helpers: `applyDaypart(articles, now)` in [news.ts](attune-app/src/services/news.ts) caps total article count by daypart (morning 06–10: 6, midday 10–17: 10, evening 17–22: 8, night 22–06: 4), and `applyTimeOfDayMood(hour)` in [prompts.ts](attune-app/src/lib/prompts.ts) returns programmatic tonality + greeting/farewell nudges per `TimeOfDay`
- ✅ **Profile match + serendipity** — `pickSerendipity(articles, declaredTopics, count=1)` in [news.ts](attune-app/src/services/news.ts) reserves 1 slot for an article whose topic is NOT in the user's profile; tagged with `selectionReason='serendipity'`. `fetchNews` automatically pulls in one extra non-profile topic feed so a serendipity candidate exists.
...
- ✅ **Full source gallery** — `SourceList.tsx` ([attune-app/src/components/SourceList.tsx](attune-app/src/components/SourceList.tsx)) renders all `NewsArticle` objects (source / date / title / link) used by the LLM as a transparent gallery, complementing per-segment `SourceLink` (added May 4)
...
- ✅ **SUS questionnaire (DE)** — full draft in `Attune_Erhebungsinstrumente.docx` §C
- ✅ **Wellbeing items / diary template** — full draft in `Attune_Erhebungsinstrumente.docx` §F ("Tagebuch")
...
- ✅ **Survey instruments (SUS, wellbeing items, diary, final interview)** — drafted in `Attune_Erhebungsinstrumente.docx` (§C SUS, §F diary, additional items in §B/D/E/G)
- ✅ **Informed consent (Outline 8.5)** — drafted in `Attune_Erhebungsinstrumente.docx` §A ("Einwilligungserklärung", §A.1–A.4)
- ✅ **Anonymization concept** — pseudonymization with separated code list, encrypted local storage, 2-year deletion policy (`Attune_Erhebungsinstrumente.docx` §A.3)
...
8. **Pilot test instruments** (SUS-DE, diary, consent, anonymization) ✅ drafted in `Attune_Erhebungsinstrumente.docx`. **In-UI survey workflow + recruitment + 1-week onboarding** still ⬜.
